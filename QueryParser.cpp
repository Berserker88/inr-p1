...
list<Posting> QueryParser::topkAndQuery(string query)
{
	size_t prev = 0;
	size_t next = 0;
	string token;
	...
	result = _dict->intersectTopk(tokens);
	return result;
}


list<Posting> QueryParser::topkParseAndExecute(string query)
{
	size_t begin = 0;
	size_t end = 0;
	...
	while ((begin = query.find_first_of('(', end)) != string::npos)
	{
		...
		if (end - begin != 0)
		{
		    ...
		    cout << "\tAND " << endl;
		    res.push_back(topkAndQuery(token));
		}
		begin = end + 1;
	}
